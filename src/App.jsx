...
async function overpassNearby(lat, lon, radiusM, tagPairs) {
  ...
  const query = `[out:json][timeout:20];(${filters});out center 15;`;
...
async function overpassEscalating(lat, lon, tagPairs, radii) {
  for (const r of radii) {
    try {
      const results = await overpassNearby(lat, lon, r, tagPairs);
      if (results.length) return { results, radius: r };
    } catch (e) { /* bir sonraki yarıçapı dene */ }
  }
  return { results: [], radius: radii[radii.length-1] };
}

function mapEmbedUrl(view, coords) {
  if (view === "local" && coords) {
  ...
  const bbox = "25.5,35.5,45.0,42.5"; // Türkiye geneli sınır kutusu
  return `https://www.openstreetmap.org/export/embed.html?bbox=${bbox}&layer=mapnik${coords?`&marker=${coords.lat},${coords.lon}`:""}`;
}
...
function speakText(text, {rate=1, pitch=1, onEnd}={}) {
  ...
  const speakNow = () => {
    ...
    // iOS Safari / bazı WebView'lerde speak() çağrısı sessizce hiçbir şey
    // yapmayabilir (motor gerçekte konuşmaya başlamaz). Kısa bir süre
    // sonra hâlâ konuşmuyorsa kullanıcıyı bilgilendiriyoruz.
    setTimeout(() => {
...
/* ============================================================
   ACTIVITIES TAB
   ============================================================ */
function PlaceCard({place, color, icon:Icon}) {
  return (
    <Card
...
function NearbyTab() {
  ...
  const [babyFallback, setBabyFallback] = useState(false);

  const loadNearby = (lat, lon) => {
    setPharmLoading(true); setBabyLoading(true);

    reverseGeocodeTR(lat, lon)
      .then(r => { setProvince(r.province); setDistrict(r.district); })
      .catch(()=>{});

    overpassEscalating(lat, lon, [["amenity","pharmacy"]], [3000, 8000, 15000])
      .then(({results}) => {
    ...
    overpassEscalating(lat, lon, [["shop","baby_goods"]], [5000, 15000, 30000])
      .then(async ({results}) => {
        let list = results, usedFallback = false;
        if (!list.length) {
          const alt = await overpassEscalating(lat, lon, [["shop","toys"]], [5000, 15000]);
          list = alt.results; usedFallback = true;
        }
        const withDist = list
          .map(p => ({...p, distanceKm: haversineKm(lat, lon, p.lat, p.lon)}))
          .sort((a,b)=>a.distanceKm-b.distanceKm).slice(0,6);
        setBabyStores(withDist); setBabyFallback(usedFallback);
      })
      .catch(()=>setBabyStores([]))
  ...
  return (
    <div style={{height:"100%",overflowY:"auto",background:"var(--bg)",padding:"20px 20px 110px"}} className="abp-scrollbar">
      ...
      <div style={{display:"flex",gap:8,marginBottom:10}}>
        <Pill_ active={mapView==="country"} onClick={()=>setMapView("country")}>Türkiye</Pill_>
      ...
      <div style={{borderRadius:"var(--radius-lg)", overflow:"hidden", boxShadow:"var(--shadow-sm)", height:200}}>
        <iframe
          ...
          src={mapEmbedUrl(mapView, coords)}
      ...
      {coords && (
        <div style={{fontSize:11, color:"var(--ink-faint)", marginTop:6, display:"flex", alignItems:"center", gap:4}}>
          <MapPin size={11}/> {district ? district+", " : ""}{province || "Konumun bulundu"}
        </div>
      )}
      ...
      {coords && (
        <>
          ...
          {pharmLoading && <SkeletonCard/>}
          {!pharmLoading && pharmacies.length===0 && (
            <Card style={{textAlign:"center", color:"var(--ink-soft)", fontSize:12.5}}>Yakında kayıtlı eczane bulunamadı.</Card>
          )}
          {pharmacies.map(p => <PlaceCard key={p.id} place={p} color="purple" icon={Pill}/>)}

          <SectionTitle>En Yakın Bebek Mağazaları</SectionTitle>
          {babyLoading && <SkeletonCard/>}
          {!babyLoading && babyStores.length===0 && (
            <Card
          ...
          {babyStores.map(p => <PlaceCard key={p.id} place={p} color="blue" icon={ShoppingBag}/>)}
          {babyFallback && babyStores.length>0 && (
            <div style={{fontSize:11,color:"var(--ink-faint)",marginTop:4}}>
              * Bebek mağazası bulunamadığından yakın oyuncak/bebek ürünleri mağazaları listelendi.
            </div>
          )}
        </>
      )}
...
/* ============================================================
   PROFILE TAB
   ============================================================ */
function ProfileTab({children, onAddChild, onOpenChildProfile, theme, setTheme, onOpenAdmin, onOpenCalendar, onLogout}) {
