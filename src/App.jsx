          ...
          return (
            <div key={m.id} style={{display:"flex",justifyContent: mine?"flex-end":"flex-start", marginBottom:10}}>
              <div className="abp-fade-up" style={{maxWidth:"78%"}}>
                {!mine && <div style={{fontSize:11,fontWeight:700,color:"var(--ink-soft)",marginBottom:2,marginLeft:4}}>{m.name}</div>}
                <div style={{
                  padding:"11px 14px", borderRadius: mine?"16px 16px 4px 16px":"16px 16px 16px 4px",
                  background: mine ? "linear-gradient(135deg, #E8A9C4, #B79AEA)" : "var(--card)",
                  color: mine ? "#fff" : "var(--ink)", fontSize:13.5, lineHeight:1.5, boxShadow:"var(--shadow-sm)", whiteSpace:"pre-wrap"
                }}>{m.text}</div>
              </div>
            </div>
          );
