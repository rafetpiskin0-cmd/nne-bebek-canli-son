...
function PoopTrackerSection({childId}) {
  ...
  const logType = async (typeKey) => {
    if (saving) return;
    setSaving(true);
    const t = STOOL_TYPES.find(s=>s.key===typeKey);
    const entry = {type:typeKey, date:new Date().toLocaleDateString("tr-TR"), time:new Date().toLocaleTimeString("tr-TR",{hour:"2-digit",minute:"2-digit"}), ts:Date.now()};
    const list = [entry, ...logs];
    const ok = await storageSet(key, list, false);
    if (ok) { setLogs(list); showToast(`${t.label} kaydedildi ✓`, t.urgent?"error":"success"); }
    else showToast("Kaydedilemedi, tekrar deneyin", "error");
    setSaving(false);
  };

  const last3Days = logs.filter(l => (Date.now()-l.ts) < 3*24*60*60*1000);
  const hardCount = last3Days.filter(l=>l.type==="sert").length;
  const showConstipationTips = hardCount >= 1 || last3Days.length === 0;
    ...
    <div style={{marginTop:16}}>
      <Card>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          {STOOL_TYPES.map(s=>(
            <div key={s.key} onClick={()=>logType(s.key)} className="abp-tap" style={{padding:"12px 8px",borderRadius:14,background:`var(--${s.color})`,fontWeight:700,fontSize:12.5,textAlign:"center"}}>{s.label}</div>
          ))}
        </div>
      </Card>

      {last3Days.length === 0 ? null : (
        <InfoBlock
          icon={hardCount>=1?AlertCircle:Check}
          color={hardCount>=1?"pink":"green"}
          title={hardCount>=1?"Son 3 Günde Sertlik Görüldü":"Son 3 Gün Normal Görünüyor"}
          text={hardCount>=1 ? "Son 3 günde sert/topak kaka kaydı var, bu kabızlık belirtisi olabilir. Aşağıdaki önerileri deneyebilirsiniz." : "Kayıtlarınıza göre kıvam normal aralıkta görünüyor, takibe devam edin."}
          highlight={hardCount>=1}
        />
      )}
      ...
      {loading ? (
        ...
        return (
          <Card key={l.ts} style={{marginBottom:8,fontSize:13.5,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div><strong>{t.label}</strong> <span style={{color:"var(--ink-faint)",fontSize:12}}>· {l.date} {l.time}</span></div>
